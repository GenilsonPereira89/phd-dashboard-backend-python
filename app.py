...
# Rota para excluir produções por período
@app.route('/api/producoes/periodo', methods=['DELETE'])
def delete_producoes_by_period():
    conn = None
    try:
        ...
        cursor.execute('DELETE FROM producoes WHERE data BETWEEN %s AND %s', (data_inicio, data_fim))
        conn.commit()
        
        if cursor.rowcount > 0:
            return jsonify({'message': f'{cursor.rowcount} registros excluídos com sucesso para o período de {data_inicio} a {data_fim}.'}), 200
        else:
            return jsonify({'message': 'Nenhum registro encontrado para o período especificado.'}), 404
    except Exception as e:
        print(f"Erro ao excluir produções por período: {e}")
        return jsonify({'error': str(e)}), 500
    finally:
        if conn:
            conn.close()

# NOVO: Rota para obter dados de produção por período (GET)
@app.route('/api/producoes/periodo', methods=['GET'])
def get_producoes_by_period():
    conn = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        sql = 'SELECT id, data, producao, is_excecao, operadores_no_dia, diaristas_no_dia FROM producoes WHERE data BETWEEN %s AND %s ORDER BY data ASC'
        cursor.execute(sql, (data_inicio, data_fim))
        producoes = cursor.fetchall()
        return jsonify(producoes)
    except Exception as e:
        print(f"Erro ao buscar produções por período: {e}")
        return jsonify({'error': str(e)}), 500
    finally:
        if conn:
            conn.close()
